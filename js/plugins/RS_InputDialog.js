...
(() => {
    ...
    RS.InputDialog.Params.variableID2 = Number(parameters["variable ID2"] || 4);

    RS.InputDialog.Params.debug = Boolean(parameters["debug"] === "true");

    RS.InputDialog.Params.localText = String(
        parameters["Account Hint"] || "Enter your account"
    );
    
    RS.InputDialog.Params.localPass = String(
        parameters["Password Hint"] || "Enter your password"
    );
    ...
    Object.assign(RS.InputDialog, {
        ...
        setRect() {
            ...
            let query, textBox, passBox, OkButton, CancelButton;
            ...
            query = `div#${RS.InputDialog.Params.szFieldId} table.inputDialogContainer tr td input[type=password]`;
            passBox = document.querySelector(query);
            ...
            if (passBox) {
                passBox.style.fontSize = Utils.isMobileDevice()
                    ? "1rem"
                    : 2 * Graphics._realScale + "em";
                passBox.style.width =
                    RS.InputDialog.getScreenWidth(
    ...
    class TextBox {
      ...
      @media screen and (min-width : 768px) and (max-width : 1000px) {
        ...
        <table class="inputDialogContainer">
              <tr class="row">
                  <td class="col">
                      ...
                      <input class="inputDialog" type="password" id"=${id} placeholder="${RS.InputDialog.Params.localPass}">
                  </td>
              </tr>
        ...
        getPassBoxId() {
            "use strict";
            const query = `div#${RS.InputDialog.Params.szFieldId} table.inputDialogContainer tr td input[type=password]`;
            return document.querySelector(query);
        }
        ...
        setRect() {
            ...
            const passBox = this.getPassBoxId();
        ...
        setPassHint() {
            const passBox = this.getPassBoxId();
            return (passBox.placeholder = RS.InputDialog.Params.localPass);
        }
    ...
    class Scene_InputDialog extends Scene_Base {
        ...
        okResult() {
            ...
            let text2 = this._textBox.getPass() || "";
            if (text2.match(/^([\d]+)$/g)) text2 = Number(RegExp.$1);
            $gameVariables.setValue(RS.InputDialog.Params.variableID2, text2);
            
            console.log($gameVariables.value(RS.InputDialog.Params.variableID));
            console.log($gameVariables.value(RS.InputDialog.Params.variableID2));
    ...
    PluginManager.registerCommand(pluginName, "pass", (args) => {
        RS.InputDialog.Params.localPass = args.text;
    });
    ...
    PluginManager.registerCommand(pluginName, "variableID2", (args) => {
        RS.InputDialog.Params.variableID2 = Number(args.number || 4);
    });
